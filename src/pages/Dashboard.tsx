...
const Dashboard = () => {
  ...
  const [upcomingEvents, setUpcomingEvents] = useState<{ id: string; titulo: string; inicio: string; tipo: string; local?: string }[]>([]);
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (metrics && typeof metrics === 'object' && !Array.isArray(metrics)) {
        const metricsObj = metrics as Record<string, number | string>;
