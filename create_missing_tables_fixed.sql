...
-- Versão corrigida que resolve conflitos de políticas existentes
...
-- 5. Remover políticas existentes para tenants (se existirem)
...
-- 6. Criar políticas RLS para tenants
...
-- 7. Remover políticas existentes para user_tenants (se existirem)
...
-- 8. Criar políticas RLS para user_tenants
...
-- 9. Atualizar políticas RLS da tabela transactions
...
-- 10. Atualizar políticas RLS da tabela payouts (se necessário)
...
-- 11. Remover função e trigger existentes (se existirem)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();

-- 12. Criar função para associar usuário ao tenant automaticamente
